  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50">
      <div className="md:hidden flex items-center justify-between p-4 bg-brand-900 text-white shadow-md">
        <div className="flex items-center space-x-2"><BookOpen className="h-6 w-6 text-brand-300" /><span className="font-bold tracking-wide">BiblioTech</span></div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-brand-950 text-slate-100 flex flex-col justify-between transition-transform duration-300 transform md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            {filteredLinks.map((link) => {
              ...
              return (
                <Link key={link.to} to={link.to} onClick={() => setSidebarOpen(false)}
                  className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 ${isActive ? 'bg-brand-500 text-white font-medium' : 'text-slate-400 hover:bg-brand-900 hover:text-slate-100'}`}>
        ...
        <div className="p-4 border-t border-brand-900">
          <div className="flex items-center space-x-3 mb-4 px-2">
            ...
            <div><p className="text-sm font-semibold truncate">{user?.name}</p><p className="text-xs text-brand-400 capitalize">{user?.role.toLowerCase()}</p></div>
          </div>
          <button onClick={handleLogout} className="w-full flex items-center justify-center space-x-2 px-4 py-2.5 bg-red-950/30 hover:bg-red-900/40 text-red-200 border border-red-900/50 rounded-xl text-sm font-medium">
